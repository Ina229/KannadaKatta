...
import { Volume2, Mic, MicOff, Play, ArrowRight, RotateCcw, Hand } from 'lucide-react';
...
const SpeakBodyPart: React.FC<SpeakBodyPartProps> = ({
  bodyParts,
  currentIndex,
  totalBodyParts,
  onNavigate,
  ...
  const playAudio = () => {
    ...
    if (currentBodyPart.audio_file) {
      const audio = new Audio(`/${currentBodyPart.audio_file}`);
      audio.play()
        .then(() => {
          console.log('Audio started playing successfully');
        })
        .catch(error => {
          console.warn('Audio file not available:', currentBodyPart.audio_file);
        })
        .finally(() => {
  ...
  const checkPronunciation = (spokenText: string) => {
    const targetText = currentBodyPart.kannada.toLowerCase().trim();
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <div className="magical-card rounded-3xl shadow-2xl p-8 max-w-3xl w-full text-center animate-glow">
        <h2 className="text-3xl font-bold gradient-text mb-6 flex items-center justify-center">
          <Hand className="h-10 w-10 mr-2 text-cyan-500 animate-sparkle" />
          Practice Speaking Body Parts
        </h2>
        ...
        {/* Instruction */}
        <div className="text-lg font-semibold text-gray-700 mb-4">
          Listen, then speak the Kannada body part name!
        </div>
        
        {/* Body Part Display */}
        <div className="bg-gradient-to-r from-cyan-200 via-blue-200 to-indigo-200 rounded-2xl p-8 mb-8 shadow-lg animate-glow">
          <div className="text-xl font-semibold text-gray-800 mb-4">
            Practice speaking this body part:
          </div>
          
          {/* Body Part Emoji */}
          <div className="text-8xl mb-6 animate-bounce-gentle">
            {getBodyPartEmoji(currentBodyPart.english)}
          </div>
          
          {/* English Body Part Name */}
          <div className="text-2xl font-bold text-gray-700 mb-6 capitalize">
            {currentBodyPart.english}
          </div>
          
          {/* Kannada Body Part Name */}
          <div className="text-4xl sm:text-6xl font-bold py-6 px-8 rounded-2xl shadow-xl mb-6 bg-gradient-to-br from-cyan-300 to-blue-300 text-cyan-800">
            {currentBodyPart.kannada}
          </div>
