...
import { Volume2, Mic, MicOff, Play, ArrowRight, RotateCcw, Hash } from 'lucide-react';
...
const SpeakNumber: React.FC<SpeakNumberProps> = ({
  numbers,
  currentIndex,
  totalNumbers,
  onNavigate,
  ...
  const playAudio = () => {
    ...
    if (currentNumber.audio_file) {
      ...
      audio.play()
        ...
        .catch(error => {
          console.warn('Audio file not available:', currentNumber.audio_file);
        })
        .finally(() => {
  ...
  const checkPronunciation = (spokenText: string) => {
    const targetText = currentNumber.kannada.toLowerCase().trim();
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <div className="magical-card rounded-3xl shadow-2xl p-8 max-w-3xl w-full text-center animate-glow">
        <h2 className="text-3xl font-bold gradient-text mb-6 flex items-center justify-center">
          <Hash className="h-10 w-10 mr-2 text-teal-500 animate-sparkle" />
          Practice Speaking Numbers
        </h2>
        
        {/* Progress Indicator */}
        <div className="text-lg font-semibold text-gray-700 mb-4">
          Number {currentIndex + 1} of {totalNumbers}
        </div>
        
        {/* Instruction */}
        <div className="text-lg font-semibold text-gray-700 mb-4">
          Listen, then speak the Kannada number!
        </div>
        
        {/* Number Display */}
        <div className="bg-gradient-to-r from-teal-200 via-cyan-200 to-blue-200 rounded-2xl p-8 mb-8 shadow-lg animate-glow">
          <div className="text-xl font-semibold text-gray-800 mb-4">
            Practice speaking this number:
          </div>
          
          {/* English Number */}
          <div className="text-4xl font-bold text-gray-700 mb-6 capitalize">
            {currentNumber.english}
          </div>
          
          {/* Kannada Number */}
          <div className="text-4xl sm:text-6xl font-bold py-6 px-8 rounded-2xl shadow-xl mb-6 bg-gradient-to-br from-teal-300 to-cyan-300 text-teal-800">
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={handleNext}
            disabled={currentIndex === totalNumbers - 1}
          ...
          >
            {currentIndex === totalNumbers - 1 ? 'Complete' : 'Next'} 
