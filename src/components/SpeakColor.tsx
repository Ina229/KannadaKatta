...
import { Volume2, Mic, MicOff, Play, ArrowRight, RotateCcw, Palette } from 'lucide-react';
...
const SpeakColor: React.FC<SpeakColorProps> = ({
  colors,
  currentIndex,
  totalColors,
  onNavigate,
  ...
  const playAudio = () => {
    ...
    if (currentColor.audio_file) {
      const audio = new Audio(`/${currentColor.audio_file}`);
      audio.play()
        .then(() => {
          console.log('Audio started playing successfully');
        })
        .catch(error => {
          console.warn('Audio file not available:', currentColor.audio_file);
        })
        .finally(() => {
  ...
  const checkPronunciation = (spokenText: string) => {
    const targetText = currentColor.kannada.toLowerCase().trim();
  ...
  const colorValue = getColorValue(currentColor.english);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <div className="magical-card rounded-3xl shadow-2xl p-8 max-w-3xl w-full text-center animate-glow">
        ...
        {/* Instruction */}
        <div className="text-lg font-semibold text-gray-700 mb-4">
          Listen, then speak the Kannada color name!
        </div>
        ...
        <div className="bg-gradient-to-r from-pink-200 via-purple-200 to-indigo-200 rounded-2xl p-8 mb-8 shadow-lg animate-glow">
          ...
          {/* Kannada Color Name */}
          <div className="text-4xl sm:text-6xl font-bold py-6 px-8 rounded-2xl shadow-xl mb-6 bg-gradient-to-br from-pink-300 to-purple-300 text-pink-800">
            {currentColor.kannada}
          </div>
